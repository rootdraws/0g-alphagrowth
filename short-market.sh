...
# ──────────────────────────────────────────────────────────────
# Create Morpho Blue Market — borrow w0G / collateral USDC.e
# LLTV: 94.5%
# ──────────────────────────────────────────────────────────────
# Usage:
#   export PRIVATE_KEY=0x...
#   ./short-market.sh <ORACLE_ADDRESS>
# ──────────────────────────────────────────────────────────────

RPC="https://evmrpc.0g.ai"
MORPHO="0x9CDD13a2212D94C4f12190cA30783B743E83C89e"

LOAN_TOKEN="0x1cd0690ff9a693f5ef2dd976660a8dafc81a109c"  #Wrapped 0g.
COLLATERAL_TOKEN="0x1f3aa82227281ca364bfb3d253b0f1af1da6473e"  #USDC.e
IRM="0xf52e20c42fec624819d4184226c4777d7cbd767e"
LLTV="945000000000000000"  # 94.5% WAD-scaled

# ── Parse oracle address argument ────────────────────────────

if [ -z "${1:-}" ]; then
  echo "ERROR: Oracle address required."
  echo "  Usage: ./short-market.sh <ORACLE_ADDRESS>"
  echo ""
  echo "  Deploy the oracle first: ./deploy-oracle-short.sh"
  exit 1
fi
...
echo "    loanToken:       $LOAN_TOKEN (w0G)"
echo "    collateralToken: $COLLATERAL_TOKEN (USDC.e)"
echo "    oracle:          $ORACLE"
echo "    irm:             $IRM"
echo "    lltv:            $LLTV (94.5%)"
echo ""
...
if [ "$LAST_UPDATE" != "0" ]; then
  ...
  echo "  Market:    SHORT — borrow w0G / collateral USDC.e / 94.5% LLTV"
  echo "  Market ID: $MARKET_ID"
  echo "  Oracle:    $ORACLE"
  echo "  Tx hash:   $TX_HASH"
